...
const DEFAULT_PATTERNS = ["\\b(?:pl|mx)-[0-9a-f]+\\b", "#\\d+\\b", "https?://\\S+"];
...
describe("loadBudget", () => {
	test("parses tracker patterns into RegExp instances", () => {
		...
		try {
			...
			expect(trackerRegexes[0]?.test("// TODO(pl-abc1): later")).toBe(true);
...
describe("scan — synthetic pass", () => {
	...
	test("placeholder strings like pl-XXXX are not flagged as markers", () => {
		const { root, cleanup } = makeFixture();
		try {
			writeFileTree(root, {
				"src/a.ts":
					"// example placeholder: pl-XXXX or mx-XXXX (no real marker)\nexport const a = 1;\n",
			});
			const budgetPath = join(root, "budget.json");
			writeBudget(budgetPath, DEFAULT_PATTERNS);
			const result = scan({
...
describe("template budget JSON", () => {
	test("budgets/debt-markers-budget.json is well-formed", () => {
		const raw = JSON.parse(
			readFileSync(resolve(REPO_ROOT, "budgets/debt-markers-budget.json"), "utf8"),
