// `plot intent <id> --goal "..." --non-goal "..."* --constraint "..."* --success-criteria "..."*`
//
...
const SPEC = {
	repeated: ["non-goal", "constraint", "success-criteria"] as const,
...
export async function runIntent(ctx: CliContext): Promise<number> {
	...
	if (!id) {
		io.err(
			"usage: plot intent <id> [--goal ...] [--non-goal ...]* [--constraint ...]* [--success-criteria ...]*\n",
		);
		return 2;
	}
	...
	if (hasFlag(args, "non-goal")) patch.non_goals = flagStringArray(args, "non-goal");
	if (hasFlag(args, "constraint")) patch.constraints = flagStringArray(args, "constraint");
	...
	if (Object.keys(patch).length === 0) {
		io.err(
			"plot intent: nothing to update (pass at least one of --goal, --non-goal, --constraint, --success-criteria)\n",
		);
		return 2;
	}
